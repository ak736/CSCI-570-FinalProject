...
def basic_sequence_alignment(S, T):
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            cost_match_mismatch = OPT[i-1][j-1] + get_mismatch_cost(S[i-1], T[j-1])
    ...
    while i > 0 or j > 0:
        ...
        if i > 0 and j > 0:
            ...
            match_mismatch_cost = OPT[i-1][j-1] + get_mismatch_cost(S[i-1], T[j-1])
...
def main(input_filepath, output_filepath):
    ...
    try:
        ...
        # Memory measurement 
        memory_consumed_kb = process_memory()
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 basic.py <input_file_path> <output_file_path>", file=sys.stderr)
        sys.exit(1)
